...
class AgentRuntime:
    ...
    async def _execute_agent(
        ...
        self._emit(
            ...
            agent.agent_id,
            task.task_id,
            f"[{agent.domain}] planning approach for: {task.objective[:120]}",
            {"domain": agent.domain, "objective": task.objective[:300]},
        )
    ...
    async def _run_leaf(
        ...
        system = pack.to_prompt_block()
        tool_names = [t["name"] for t in pack.tool_contracts] if pack.tool_contracts else []
        tool_hint = ""
        if tool_names:
            tool_hint = (
            ...
            for tc in response.tool_calls:
                ...
                for src in (out.data or {}).get("sources") or []:
                    ...
                    self._emit(
                        ...
                        agent.agent_id,
                        task.task_id,
                        src.get("title") or src.get("url") or "source",
                        src,
                    )
        ...
        if sources:
            ...
            for s in sources[:8]:
                ...
                url = s.get("url") or ""
                cite_lines.append(f"- [{title}]({url})" if url else f"- {title}")
