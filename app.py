...
def get_services():
    if "services" not in st.session_state:
        ...
        tool_reg.register(
            PinchTabTool(get_base_url=_pt_url, get_token=_pt_token)
        )
        tool_reg.register(AgentReachTool())
        tool_reg.register(MemoryTool(project_root=str(root)))
        tool_reg.register(CodeGraphTool(root))
        tool_reg.register(DefuddleTool())
        tool_reg.register(HumanizeTool())
...
def main():
    ...
    with st.sidebar:
        ...
        if provider == "ollama":
            ...
            st.info(
                ...
                "On Streamlit Cloud use an ngrok/public URL like `https://xxxx.ngrok.io/v1`."
            )
        ...
        with col_c:
            if st.button("Test connection", use_container_width=True):
                try:
                    ...
                    from src.models.provider import ProviderConfig as _PC
                    _cfg = _PC(provider=provider, model="probe", base_url=base_url.strip() or None, credential_ref=provider)
                    client = services["providers"].create(_cfg, cfg_key)
                    if hasattr(client, "ping"):
                        result = run_async(client.ping())
                        if hasattr(result, "__await__") or __import__("asyncio").isfuture(result):
        ...
        run_mode = st.selectbox(
            ...
            index=0,
            key="run_mode_select",
            help="plan = read/search/web only, produce structured plan (no write/edit/bash). agent = full execution.",
        )
        ...
        pt_url = st.text_input(
            ...
            help="PinchTab HTTP API base (default port 9867). Start with: pinchtab server",
            key="pt_url_input",
        )
        if pt_url:
        ...
        pt_tok = st.text_input(
            ...
            help="Only if server requires PINCHTAB_TOKEN / auth",
            key="pt_token_input",
        )
        if pt_tok:
    ...
    mode_label = "PLAN" if session.mode == "plan" else "AGENT"
    st.markdown(
        f"""
        <div class="header-bar">
            <h1>AgentForge</h1>
            <span class="header-meta">session {session.session_id[:8]} · {provider}/{model} · {mode_label} · Streamlit (temp)</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
    st.caption(
        "Primary UI is FastAPI liquid-glass at `/ui` (uvicorn backend). "
        "Streamlit remains for quick local checks of the agent runtime + tools."
    )
    ...
    with col_side:
        ...
        st.caption(
            ...
            "(git, code-review, implementation, …) run in parallel → results merge."
        )
        domains_avail = [
